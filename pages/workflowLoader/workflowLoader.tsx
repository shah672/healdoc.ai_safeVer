...
import Sidebar, {SidebarLeft} from '../Sidebar';
...
//import all sidebars 
import RecordPatient from '../../components/sidebars/RecordPatient';
...
import styles from './workflowLoader.module.css';
...
const WorkflowLoader = ( ) => {
    ...
    //there can be some issue in the setelements code too.
    ...
    useEffect(() => {
        ...
        try {
            const workflowData = JSON.parse(Array.isArray(router.query.workflow) ? router.query.workflow[0] : router.query.workflow);
            console.log('Parsed Workflow Data:', workflowData);
        
            if (workflowData) {
                const parsedState = JSON.parse(workflowData.state);
                console.log('Setting elements:', workflowData.parsedState);
                console.log('Setting plusIndex:', workflowData.parsedState);
                setElements(parsedState.elements || []);
                setShowDropdown(parsedState.showDropdown || false);
                setSelectedIndex(parsedState.selectedIndex || null);
                setPlusIndex(parsedState.plusIndex || -1);
                setSidebar1Open(parsedState.sidebar1Open || true);
                setWorkflowName(parsedState.name || "");
                setLastModified(parsedState.lastModified || "");
